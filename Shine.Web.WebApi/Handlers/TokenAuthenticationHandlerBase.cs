...
namespace Shine.Web.WebApi.Handlers
{
    public abstract class TokenAuthenticationHandlerBase : DelegatingHandler
    {
        ...
        {
            if (!request.Headers.Contains(HttpHeaderNames.ShineAuthenticationToken))
            {
                return base.SendAsync(request, cancellationToken);
            }
            string authenticationToken = request.Headers.GetValues(HttpHeaderNames.ShineAuthenticationToken).First();
