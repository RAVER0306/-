...
namespace Shine.Comman.Extensions
{
    /// <summary>
    /// Try-Catch扩展操作
    /// </summary>
    public static class TryCatchExtensions
    {
        ...
        /// <typeparam name="TResult">返回值类型</typeparam>
        /// <param name="source">值</param>
        /// <param name="check">参数检测方法</param>
        /// <param name="failureAction">失败处理方法</param>
        /// <param name="successAction">要对值执行的主功能代码</param>
        /// <returns>功能代码的返回值，如果出现异常，则返回对象类型的失败方法处理的默认值</returns>
        public static  TResult TryCatch<T, TResult>(this T source, Action<T> check, Func<Exception, TResult> failureAction, Func<T, TResult> successAction)
           where T : class
        {
            TResult result;
            try
